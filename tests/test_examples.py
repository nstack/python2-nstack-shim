...
def test_process_schema():
    a, b, c = build.process_schema(example)
...
def test_nested_sum():
    a, b, c = build.process_schema({'types': [['Foo', t_sum(('A', t_int), ('B', t_bool))], ],
                                    'signatures': {'method1': [
...
def test_sum_args():
    a, b, c = build.process_schema({'types': [['Foo', t_sum(('A', t_sum(('C', t_int), ('D', t_bool))),
...
def test_unit_input():
    a, b, c = build.process_schema({'signatures': {'method1': [t_unit, t_bool]}})
    x = b(_object_with_method((lambda self, i: i == ()), name='method1'))
    assert x.method1(()) is True

def test_unit_output():
    a, b, c = build.process_schema({'signatures': {'method1': [optional(t_unit), t_unit]}})
    x = b(_object_with_method((lambda self, i: ()), name='method1'))
    assert x.method1(None) == ()

def test_lists():
    a, b, c = build.process_schema({'signatures': {'method1': [t_array(t_int), t_int]}})
